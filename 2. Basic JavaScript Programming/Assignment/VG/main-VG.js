...
function getCoordinates(tile) {
  /* TODO */
  return {row: parseInt(tile.attributes[1].value), col: parseInt(tile.attributes[2].value)};
}
...
function removeShip(ship) {
  /* TODO */
  tiles.forEach((tile) => {
    if (ship.contains(getCoordinates(tile))) {
      clearTile(tile);
    }
  })
}
...
// (tip: use addMark)
function displayMarkersOnGrid(markers) { // [...markedHits, ...markedBoms], []
  tiles.forEach((tile) => {
...
function displayHitsAndBoms(player) { // players.enemy
  clearGrid();
...
// Check test.js for specification of how it should work
function registerHitOrBom(guess, player) { // players.enemy
  /* TODO */
  let hit = false;
